...
public class CustScrollView extends ScrollView {
    boolean allowDragBottom = true; // 如果是true，则允许拖动至底部的下一�?
    float downY = 0;
    boolean needConsumeTouch = true; // 是否�?要承包touch事件，needConsumeTouch�?旦被定�?�，则不会更�?
    int maxScroll = -1; // �?大滑动距�?
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            if (!needConsumeTouch) {
                // 在最顶端且向上拉了，则这个touch事件交给父类去处�?
                getParent().requestDisallowInterceptTouchEvent(false);
                return false;
            } else if (allowDragBottom) {
                // needConsumeTouch尚未被定性，此处给其定�??
